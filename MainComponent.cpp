...
void MainComponent::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    // Near end-of-file – apply ultra-gentle exponential ramp-down over the final few blocks + hard zero for absolute void silence
    double secondsToEnd = transportSource.getLengthInSeconds() - transportSource.getCurrentPosition();
    double blockDuration = bufferToFill.numSamples / currentSampleRate;
    if (secondsToEnd <= blockDuration * 4.0) // Trigger ~4 blocks early (~0.09 s)
    {
        float progress = static_cast<float>(secondsToEnd / (blockDuration * 4.0)); // 1.0 → 0.0 over 4 blocks
        float expDecay = std::pow(progress, 3.0f); // Gentle exponential curve (cubic power for smoothness)
        for (int ch = 0; ch < bufferToFill.buffer->getNumChannels(); ++ch)
        {
            auto* data = bufferToFill.buffer->getWritePointer(ch, bufferToFill.startSample);
            for (int i = 0; i < bufferToFill.numSamples; ++i)
            ...
            // Hard zero the absolute last sample of the final block
            if (secondsToEnd <= blockDuration)
                data[bufferToFill.numSamples - 1] = 0.0f;
        }
    }
